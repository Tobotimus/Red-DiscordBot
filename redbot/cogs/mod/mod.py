...
class Mod(
    ...
    def __init__(self, bot: Red):
        ...
        self.settings.guild.register(**self.default_guild_settings)
        self.settings.channel.register(**self.default_channel_settings)
        self.settings.member.register(**self.default_member_settings)
        self.settings.user.register(**self.default_user_settings)
    ...
    async def _maybe_update_config(self):
        ...
        if await self.settings.version():
            return
        guild_dict = await self.settings.guild.all()
        for guild_id, info in guild_dict.items():
            ...
            if delete_repeats:
                ...
                val = -1
            await self.settings.guild(guild_id).delete_repeats.set(val)
