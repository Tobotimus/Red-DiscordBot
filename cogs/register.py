...
class Register:
    ...
    async def _regedit_addrole(self, ctx, *, role_name: str):
        ...
        try:
            if role.id not in self.json[server.id]:
                ...
                dataIO.save_json(self.location, self.json)
                await self.bot.say('``{}`` is now a valid register role.'.format(role.name))
            else:
                # ROLE ALREADY IN REGISTER
                await self.bot.say('``{}`` is already a valid register role!'.format(role.name))
        except:
    ...
    async def _regedit_removerole(self, ctx, *, role_name: str):
        ...
        try:
            role = discord.utils.get(server.roles, name=role_name)
            if role.id in self.json[server.id]:
                del self.json[ctx.message.server.id][role.id]
                dataIO.save_json(self.location, self.json)
                await self.bot.say('``{}`` is no longer a valid register role.'.format(role.name))
        except:
            await self.bot.say('That role isn\'t in this server.')
